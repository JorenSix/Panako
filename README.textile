...
Panako contains a "lightweight HTTP server":http://acme.com/java/software/Acme.Serve.Serve.html to provide a REST webservice. The REST webservice provides two 
...
<dt>Version 2.0</dt><dd>2021-04-27</dd> This release add support for LMDB a faster key-value store. A new implementation of the classic Shazam algorithm is included with a near-neigbour search fingerprint. The version bumped to 2.0 to signify incompatibility with older JRE's: lambdas are used so a newer JRE (8+) is needed.
