...
<dt>Version 2.0</dt><dd>2021-04-27</dd> A major overhaul of Panako. The main aim of this release is to ensure the longevity of Panako. The featureset has been reduced to focus on core functionality. The version bump is also explained by the use of lambdas and the need for a newer JRE (8+). A list of changes: 
* The number of dependencies has been drastically cut by removing support for multiple key-value stores. 
* The main key-value store has been changed to a faster and simpler system (from MapDB to LMDB). 
...
** The Gaborator algorithm is a new version of the algoritm described in the Panako paper. It uses the "Gabor transform":https://en.wikipedia.org/wiki/Gabor_transform to go from time domain to the spectral domain. It is implemented by "JGaborator":https://github.com/JorenSix/JGaborator which in turn relies on "The Gaborator":https://gaborator.com/ library. 
* Folder structure has been simplified.
* with the addition of a number of helper scripts.
* Moved default panako location to ~/.panako/panako
